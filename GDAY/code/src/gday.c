...
void spin_up_annual(control *c, fluxes *f, met *m,
    ...
    */
    double tol_c = 1E-02; //tol_c = 1E-06;
    double tol_n = 1E-02; //tol_n = 1E-06;
    double tol_p = 1E-02; //tol_p = 1E-04;
          ...
          break;
            ...
            for(moy = 0; moy < 12; moy++) {
                ...
                if (c->pcycle) {
                  ...
                  fprintf(stderr,
                          "Spinup: Iteration %d, moy %d, Leaf NC %f, Leaf PC %f, NPP %f, stem %f\n",
                          year, moy, s->shootnc, s->shootpc, f->npp, s->stem);
                } else if (c->ncycle) {
                  /* Have we reached a steady state? */
                  fprintf(stderr,
