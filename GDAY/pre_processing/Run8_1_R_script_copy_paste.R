...
Run8_1_copy_paste <- function() {
    ...
    out[32] <- "    param_dir <- paste0(d, \"/params/Run8_1\")"
    
    # swap run folder locations
    out[33] <- "    run_dir <- paste0(d, \"/outputs/Run8_1\")" 
    ...
    # write
    writeLines(out, "GDAY/simulations/Run8_1/quasi_equil_annual_spin_up.R")
    
    ### read in simulation file
    tran <- readLines("GDAY/simulations/Run1/quasi_equil_annual_simulations.R")
    
    # swap locations
    tran[30] <- "    param_dir <- paste0(d, \"/params/Run8_1\")"  
    tran[32] <- "    run_dir <- paste0(d, \"/outputs/Run8_1\")"
    
    
    # write
    writeLines(tran, "GDAY/simulations/Run8_1/quasi_equil_annual_simulations.R")
