...
# Find the medium term equilibrium nf and NPP under standard conditions - by finding the root
# specifically for exudation model, without considering exudation
solveMedium_full_cnp <- function(CO2,Cpass,Cslow,NinL) {
    fn <- function(nf) {
        photo_constraint_full_cnp(nf, inferpfM(nf, allocn(nf), PinM = Pin+PrelwoodVLong,
                                               NinM = Nin+NrelwoodVLong,
                                               CpassL=Cpass, CpassM=Cslow), allocn(nf), 
                                  allocp(inferpfM(nf, allocn(nf), PinM = Pin+PrelwoodVLong,
                                                  NinM = Nin+NrelwoodVLong,
                                                  CpassL=Cpass, CpassM=Cslow)),
                                 CO2) - NConsMedium(nf,allocn(nf),Cpass,Cslow,NinL)$NPP
        
    }
    equilnf <- uniroot(fn,interval=c(0.01,0.1))$root
    equilNPP <- photo_constraint_full_cnp(equilnf, 
                                         allocn(equilnf), CO2)
    ans <- data.frame(equilnf,equilNPP)
    return(ans)
}
