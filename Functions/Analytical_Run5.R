...
Perform_Analytical_Run5 <- function(plotting = T) {
    ...
    # Find equilibrate intersection and plot
    LongN <- solveLongN_respiration(co2=CO2_2, Cpass=CpassVLong, Nin=0.4+NrelwoodVLong, nwvar=F)
    ...
    # store constraint and equil DF onto their respective output df
    cDF[cDF$Run == 5 & cDF$CO2 == 700, 3:13] <- out350DF
    eDF[eDF$Run == 5 & eDF$CO2 == 700, 3:8] <- equil350DF
